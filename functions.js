...
//		td.style.backgroundColor = "#FFFFFF";
	...
	function changePage( obj ){
		...
		for ( var i = 0 ; i < bars.length ; i++ ){
			if ( i == index ){
				bars[i].style.backgroundColor = "#E2A98A";
				bars[i].style.fontWeight = "bold";
				mains[i].style.display = "block";
			} else {
				bars[i].style.backgroundColor = "#F4D9C7";
...
//									singleCardHTML += theText;
//									singleCardHTML += getAbilityWithTag[abo].innerText;
									singleCardHTML += getAbilityWithTag[abo].outerHTML.replace( "<span>"+getAbilityWithTag[abo].innerText+"</span>", "<span>"+theText+"</span>" );
								}
							}
							if ( abl > 0 ){
								singleCardHTML += "</li>";
...
//		console.log( gobi( "list_block" ).src );
	...
	function importGachi( ignore ){
		if ( ignore ){
			alert( "請將ガチまとめ的牌組網址私訊給系統管理員，系統管理員有空的時候會幫您手動產生專屬翻譯用網址" );
		} else {
